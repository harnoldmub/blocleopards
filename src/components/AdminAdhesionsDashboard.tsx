...
function Drawer({ row, onClose, onUpdate, onDelete }: { row: any; onClose: () => void; onUpdate: (id: string, status: string, notes: string) => Promise<void>; onDelete: (id: string) => Promise<void> }) {
  ...
  const generateDraft = async () => {
    ...
    setFeedback(null);
    try {
      const r = await fetch("/api/admin/adhesion-ai-draft", {
        method: "POST",
  ...
  const sendReply = async () => {
    ...
    setFeedback(null);
    try {
      const r = await fetch("/api/admin/adhesion-reply", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ id: row.id, to: row.email, body: reply, notes }),
      });
      const d = await r.json();
      if (r.ok && d.success) {
        setFeedback({ type: "ok", text: "Réponse envoyée par email." });
